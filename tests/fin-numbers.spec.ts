...
class FinancialTestHelper {
  ...
  async getDisasterOptions(): Promise<string[]> {
    ...
    const options = await this.page.$$eval('#ts-filter-disaster-panel .mat-mdc-option', 
  ...
  async navigateToHome() {
    await this.page.keyboard.press('Escape');
    await this.page.waitForTimeout(100);
    //click first div with tabindex=0. there are multiple divs with tabindex=0, so make sure we pick the first one
    await this.page.locator('div[tabindex="0"]').nth(0).click();
    //scroll down
